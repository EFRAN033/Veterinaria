<template>
  <section class="bg-white py-16 px-4 md:px-8 max-w-7xl mx-auto">
    ...
    <div class="text-center mb-12">
      <h2 class="app-type-title text-gray-800 mb-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      <div 
        ...
        :key="pet.id"
        class="bg-white rounded-none overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2 border border-gray-100"
      >
        <!-- Image Container -->
        ...
        <div class="p-6">
          <div class="flex justify-between items-start mb-2">
            <h3 class="app-type-panel-heading text-gray-800">{{ pet.name }}</h3>
            <span class="app-type-stat" :title="pet.gender === 'Macho' ? 'Macho' : 'Hembra'">
              {{ pet.gender === 'Macho' ? '♂' : '♀' }}
            </span>
          </div>
          ...
          <button @click="openAdoptionModal(pet)" class="w-full py-3 rounded-none border-2 border-[#BEDC74] text-[#1a5f63] font-bold hover:bg-[#BEDC74] hover:text-white transition-colors duration-300 flex items-center justify-center gap-2">
            <span>Adoptar</span>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="bg-white rounded-none shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-fade-in-up">
        <div class="bg-[#1BB0B9] p-6 text-white text-center relative">
          ...
          <h3 class="app-type-panel-heading mb-1">¡Gracias por tu interés!</h3>
        ...
        <div class="p-8 text-center">
          ...
          <div class="space-y-3">
            <a :href="`tel:${selectedPet?.phone}`" class="block w-full py-3 rounded-none bg-[#BEDC74] text-[#1a5f63] font-bold hover:bg-[#d4ed95] transition-colors">
              Llamar ahora
            </a>
            <button @click="showModal = false" class="block w-full py-3 rounded-none border-2 border-gray-100 text-gray-500 font-bold hover:bg-gray-50 transition-colors">
              Seguir mirando
            </button>
          </div>
        </div>
